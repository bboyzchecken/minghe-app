 ...
 *
 * ทำงานได้ทั้งโหมด mock (คิวจำลอง + แอดมินสมมุติอีกคนถืองานอยู่) และ live (Go API)
 */
...
function AdminConsole() {
  ...
  const [legal, setLegal] = useState<AdminLegalDoc[] | null>(null)
  ...
  const reload = useCallback(async () => {
    ...
    setError(null)
    try {
      const [ov, od, us, lg] = await Promise.all([
        client.adminOverview(token),
        client.adminListOrders(token),
        client.adminListUsers(token),
        client.adminListLegal(token),
      ])
      setOverview(ov)
      setOrders(od)
      setUsers(us)
  ...
  /** ทุก action ผ่านทางเดียว: ทำ → โหลดใหม่ → ถ้าโดนกัน (งานของคนอื่น) แสดงเหตุผล */
  async function act(fn: () => Promise<void>, successNote: string) {
    setNotice(null)
    setError(null)
    try {
      await fn()
      ...
      await reload() // สถานะอาจถูกแอดมินคนอื่นเปลี่ยนไปแล้ว — ดึงของจริงมาแสดง
    }
  }

  const myTasks = (orders ?? []).filter((o) => o.assigneeIsMe && o.status !== 'delivered')
    ...
    <div className="pb-16">
      ...
      <div className="bg-ink text-paper">
        <div className="container-page flex flex-wrap items-center justify-between gap-3 py-5">
          ...
          <button onClick={() => void reload()} className="btn border border-paper/30 !py-2 text-sm text-paper hover:bg-paper/10">
            ⟳ รีเฟรชข้อมูล
          </button>
        </div>
      ...
      <div className="container-page pt-6">
        ...
        {tab === 'overview' && <OverviewTab overview={overview} myTasks={myTasks} goQueue={() => setTab('queue')} />}
        {tab === 'queue' && <QueueTab orders={orders} act={act} />}
        {tab === 'users' && <UsersTab users={users} meEmail={user?.email ?? ''} act={act} />}
        ...
        <p className="mt-8 text-center text-xs text-muted">
          {IS_MOCK
            ...
            : 'โหมดใช้งานจริง — ข้อมูลจากฐานข้อมูล การรับเรื่องมีผลกับแอดมินทุกคนทันที'}
        </p>
      </div>
    </div>
  )
...
function QueueTab({
  ...
  act: (fn: () => Promise<void>, note: string) => Promise<void>
}) {
  const { token } = useSession()
  const [filter, setFilter] = useState<QueueFilter>('all')

  if (!orders || !token) return <Skeleton rows={4} />
            ...
            <tbody>
              {rows.map((o) => (
                <tr key={o.id} className={`border-b border-line/60 last:border-0 ${o.assigneeIsMe ? 'bg-gold/[0.04]' : ''}`}>
                  ...
                  <td className="py-3">
                    <RowActions order={o} act={act} token={token} />
                  </td>
                </tr>
              ))}
            </tbody>
...
function RowActions({
  ...
  if (order.assignee && !order.assigneeIsMe) {
    return (
      <button
        onClick={() => void act(() => client.adminReleaseOrder(token, order.id), `คืนงาน ${order.code} เข้าคิวกลางแล้ว`)}
        className="text-xs text-muted underline-offset-2 hover:text-terracotta hover:underline"
  ...
  return (
    <div className="flex flex-wrap gap-1.5">
      {!order.assignee && (
        <button
          onClick={() => void act(() => client.adminClaimOrder(token, order.id), `รับเรื่อง ${order.code} แล้ว — งานนี้เป็นของคุณ`)}
          className="rounded-md border border-gold/50 bg-gold/[0.08] px-2.5 py-1 text-xs font-medium text-gold hover:bg-gold/[0.15]"
        >
          รับเรื่อง
        </button>
      )}
      {order.status === 'paid' && (
        <button
          onClick={() => void act(() => client.adminProcessOrder(token, order.id), `เริ่มดำเนินการ ${order.code} แล้ว`)}
          className="rounded-md border border-line bg-cloud px-2.5 py-1 text-xs text-ink hover:border-gold/40"
        >
          เริ่มดำเนินการ
        </button>
      )}
      {order.status === 'processing' && order.assigneeIsMe && (
        <button
          onClick={() => void act(() => client.adminDeliverOrder(token, order.id), `ส่งมอบ ${order.code} เรียบร้อย`)}
          className="rounded-md border border-jade/50 bg-jade/[0.08] px-2.5 py-1 text-xs font-medium text-jade hover:bg-jade/[0.15]"
        >
          ส่งมอบ
        </button>
      )}
      {order.assigneeIsMe && order.status !== 'processing' && (
        <button
          onClick={() => void act(() => client.adminReleaseOrder(token, order.id), `คืนงาน ${order.code} เข้าคิวกลางแล้ว`)}
          className="px-1 text-xs text-muted hover:text-terracotta"
        >
          คืนงาน
        </button>
      )}
...
function UsersTab({
  ...
  const { token } = useSession()
  if (!users || !token) return <Skeleton rows={4} />
              ...
              <tr key={u.id} className="border-b border-line/60 last:border-0">
                ...
                <td className="py-3">
                  {u.email === meEmail ? (
                    ...
                    <button
                      onClick={() => void act(() => client.adminSetUserStatus(token, u.id, 'deactivated'), `ระงับบัญชี ${u.email} แล้ว`)}
                    ...
                    <button
                      onClick={() => void act(() => client.adminSetUserStatus(token, u.id, 'active'), `คืนสิทธิ์ ${u.email} แล้ว`)}
                      className="text-xs text-jade underline-offset-2 hover:underline"
                    >
                      คืนสิทธิ์
                    </button>
                  )}
...
function LegalTab({ legal }: { legal: AdminLegalDoc[] | null }) {
