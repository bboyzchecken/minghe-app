...
async function call<T>(
  ...
  try {
    ...
    throw new ClientError(
      `ติดต่อ API ไม่ได้ที่ ${API_BASE_URL} — ตรวจว่ารันเซิร์ฟเวอร์แล้วหรือยัง (หรือสลับกลับเป็นโหมด mock)`,
    )
  }
...
export const liveClient: MingheClient = {
  ...
  async login(email, password) {
    const res = await call<{ token: string; user: { id: number; email: string; name: string; role: string } }>(
      '/auth/login',
      { method: 'POST', body: { email, password } },
    )
    const resolved = await resolveSide(res.token, res.user.role, res.user.id)
    return {
      token: res.token,
      user: {
  ...
  async me(token) {
    const user = await call<{ id: number; email: string; name: string; role: string }>('/api/me', {
      token,
    })
  ...
  async createOrder(token, draft) {
    ...
    const paid = await call<ApiOrder>(`/api/orders/${created.id}/pay`, {
      method: 'POST',
      token,
      body: { consent_id: consent.id, method: 'mock', payment_ref: `DEMO-${created.code}` },
    })
